...
arma::mat proximity(
  ...
  int N = dat.nrow()*(dat.nrow()-1);
  Progress p(N,verbose); // track progress
...
# set.seed(123)
# N = 100
# dates = seq(from = as.Date("2007-01-01"),to=as.Date("2007-01-07"),by = "day")
# lons = runif(N,0,.1)
# lats = runif(N,0,.1)
# d1 = data.frame(source = 1, date = sample(dates,size = N,replace = T),latitude = sample(lats,N,T),longitude = sample(lons,N,T))
# d2 = data.frame(source = 2, date = sample(dates,size = N,replace = T),latitude = sample(lats,N,T),longitude = sample(lons,N,T))
# d3 = data.frame(source = 3, date = sample(dates,size = N,replace = T),latitude = sample(lats,N,T),longitude = sample(lons,N,T))
# D = rbind(rbind(d1,d2),d3)
# D = data.matrix(D)
#
# P = locate_proximity(dat = D,t = 1,s=1)[-1,]
# head(P)
# P
# loc=4
